...
function Resetpassword(){
    ...
    const handleSubmitfinal = async (eventObj)=>{
        ...
        if(newpassword===conpassword){
            const resetResponse = await fetch('http://localhost:3000/reset-response', {
                method:'POST',
    ...
    <div className="p-8 bg-white rounded-lg shadow-2xl">
        ...
        <form onSubmit={handleSubmitfinal} className="space-y-6">
            ...
            <div>
                <label className="block text-sm font-semibold mb-2" htmlFor="conpassword">Confirm Password</label>
                <div className="flex items-center border rounded-lg shadow-sm overflow-hidden">
                    <span className="pl-3">
                        <i className="fas fa-lock text-gray-500"></i>
                    </span>
