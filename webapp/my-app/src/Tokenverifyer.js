...
function Tokenverifyer() {
  const [token, setToken] = useState("");
  const { email, adminVal } = useParams();

  const navigate = useNavigate();

  const submitHandler = async (eventObj) => {
    eventObj.preventDefault();
    const response = await fetch("http://localhost:3000/token-for-ver", {
      method: "POST",
    ...
    if (response.ok) {
      if (data.verified) {
        alert(data.message);
        navigate(`/profile/resetpassword/${email}/${adminVal}`);
      } else {
        alert(data.message);
      }
    }
